 ...
 *   - 修复点击悬浮球菜单项无反应：改用left/top定位替代transform定位
 *   - 自定义图片改为图床链接输入，移除file上传
 *   - 快捷方式支持角色头像或自定义图标URL
 *   - 新增unmountAppView()和destroyBall()方法
 *
 * v1.0.5 更新：
...
;(function () {
  ...
  p._getMenuItems = function() {
    ...
    ;(this.config.shortcuts || []).forEach(function(sc) {
      var menuIcon = sc.iconUrl ? '<img src="' + escHtml(sc.iconUrl) + '" style="width:20px;height:20px;border-radius:50%;object-fit:cover;">' : (sc.icon || svgIcon('link'))
      items.push({ id: sc.id, name: sc.name, icon: menuIcon, type: sc.type, action: function() { self.closeMenu(); self._executeShortcut(sc) } })
    })
  ...
  p._renderAppView = function() {
    ...
    app.innerHTML = ''
      ...
      + '  <div class="rh-row">'
      + '    <div style="flex:1"><div class="rh-row-label">\u81EA\u5B9A\u4E49\u56FE\u7247</div>'
      + '    <input class="rh-input" id="rh-img-url" value="' + escHtml(cfg.ball.customImage || '') + '" placeholder="\u8F93\u5165\u56FE\u7247\u94FE\u63A5" style="margin-top:6px;font-size:12px;"></div>'
      + '  </div>'
  ...
  p._bindAppEvents = function(app) {
    ...
    imgUrl.addEventListener('change', async function() {
      ...
      self.config.ball.customImage = url
      await self._saveConfig()
      self._applyBallStyle(self.ballEl)
      var inner = self.ballEl.querySelector('.roche-hub-ball-inner')
      if (inner) {
        if (url) { inner.style.backgroundImage = 'url(' + url + ')'; inner.innerHTML = '' }
        else { inner.style.backgroundImage = ''; inner.innerHTML = '<span class="roche-hub-ball-icon">' + svgIcon('star') + '</span>' }
      }
    })
    ...
    hbToggle.addEventListener('change', async function() {
      ...
      if (hbToggle.checked) hbTrack.classList.add('on')
      else hbTrack.classList.remove('on')
      await self._saveConfig()
  ...
  p._renderShortcutList = function() {
    ...
    listEl.innerHTML = shorts.map(function(sc, idx) {
      var ic = sc.iconUrl ? '<img src="' + escHtml(sc.iconUrl) + '" style="width:20px;height:20px;border-radius:50%;object-fit:cover;">' : (icMap[sc.type] || svgIcon('link'))
  ...
  p._showShortcutModal = function(editIdx) {
    ...
    var sc = isEdit ? this.config.shortcuts[editIdx] : { id: uid(), name: '', type: 'character', targetId: '', targetName: '', icon: '', iconUrl: '', customAction: '' }
    var self = this
    ...
    ;(this._charList || []).forEach(function(ch) {
      ...
      charOptions += '<option value="' + escHtml(cid) + '"' + sel + ' data-avatar="' + escHtml(ch.avatarUrl || ch.avatar || '') + '">' + escHtml(cname) + '</option>'
    })
    ...
    ;(this._appList || []).forEach(function(ap) {
      ...
      var aname = ap.handle || ap.name || ap.title || aid
      if (ap.isGroup) aname = '\uD83D\uDCAC ' + aname
    ...
    mask.innerHTML = '<div class="rh-modal">'
      ...
      + '      <option value="app"' + (sc.type === 'app' ? ' selected' : '') + '>\uD83D\uDCAC \u4F1A\u8BDD\u8DF3\u8F6C</option>'
      ...
      + '    <label class="rh-fg-label">\u9009\u62E9\u4F1A\u8BDD</label>'
      + '    <select class="rh-select" id="ms-target-app">' + appOptions + '</select>'
      + '    <div class="rh-hint">\u4ECE\u5DF2\u6709\u7684\u4F1A\u8BDD\u5217\u8868\u4E2D\u9009\u62E9\uFF0C\u5305\u542B\u5355\u804A\u548C\u7FA4\u804A\u3002</div>'
      + '  </div>'
      ...
      + '  <div class="rh-fg"><label class="rh-fg-label">\u56FE\u6807\u56FE\u7247\u94FE\u63A5</label>'
      + '    <input class="rh-input" id="ms-icon-url" value="' + escHtml(sc.iconUrl || '') + '" placeholder="\u89D2\u8272\u5934\u50CF\u6216\u81EA\u5B9A\u4E49\u56FE\u7247URL"></div>'
    ...
    targetSel.addEventListener('change', function() {
      ...
      if (opt && opt.dataset.avatar) {
        mask.querySelector('#ms-icon-url').value = opt.dataset.avatar
      }
    })
    ...
    mask.querySelector('.ms-save').addEventListener('click', async function() {
      ...
      var data = {
        ...
        iconUrl: mask.querySelector('#ms-icon-url').value.trim(),
    ...
    apps: [{
      ...
      mount: async function(container, roche) {
        if (!_hubInstance) {
          ...
          _hubInstance._container = container
          _hubInstance._renderAppView()
        }
      },
      unmount: async function(container, roche) {
        if (_hubInstance) {
          _hubInstance.unmountAppView()
        }
      },
    }],
  })
